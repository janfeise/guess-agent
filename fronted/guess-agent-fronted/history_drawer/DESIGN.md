...
Our palette relies on a sophisticated interplay of mint greens and soft cyans, now accented with a vibrant violet, all anchored by a rich "on-surface" slate.
...
- **Agent Bubble:** Use `tertiary-container` (Pastel Violet, representing the updated tertiary color) with `xl` rounding on all corners except the bottom-left.
